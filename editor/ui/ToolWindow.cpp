...
{
	ImGui::Begin(u8"¹¤¾ß", &Windows::ShowToolWindow, ImGuiWindowFlags_NoDocking);
	ImGui::ShowStyleEditor();
	ImGui::End();
}

void ToolWindow::helpMarker(const char* desc)
{
	ImGui::TextDisabled("(?)");
...
void ToolWindow::nodeFont(ImFont* font)
{
...
void ToolWindow::showStyleEditor(ImGuiStyle* ref)
{
		...
		if (ImGui::BeginTabItem("Colors"))
		{
			...
			if (ImGui::Button("Export"))
			{
				if (output_dest == 0)
					...
					ImGui::LogToTTY();
				ImGui::LogText("ImVec4* colors = ImGui::GetStyle().Colors;" NB_NEWLINE);
				for (int i = 0; i < ImGuiCol_COUNT; i++)
				{
					const ImVec4& col = style.Colors[i];
					const char* name = ImGui::GetStyleColorName(i);
					if (!output_only_modified || memcmp(&col, &ref->Colors[i], sizeof(ImVec4)) != 0)
						ImGui::LogText("colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" NB_NEWLINE,
